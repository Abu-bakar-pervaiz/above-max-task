                ...
                <form action="{{ route('employee.update',['employee'=>$employee->id]) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex gap-x-4 flex-wrap justify-between">
                        <div class="mb-4 w-full md:w-[47%]">
                            ...
                            <input type="text" required value="{{ $employee->first_name }}" id="first-name" minlength="3" name="first_name"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div class="mb-4 w-full md:w-[47%]">
                            <label for="last-name" class="block text-sm font-medium text-gray-700">Last Name</label>
                            <input type="text" required value="{{ $employee->last_name }}" id="last-name" minlength="3" name="last_name"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div class="mb-4 w-full md:w-[47%]">
                        ...
                        <div class="mb-4 w-full md:w-[47%]">
                            ...
                            <input type="email" value="{{ $employee->email }}" id="email" name="email"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                        <div class="mb-4 w-full">
                            <label for="phone" class="block text-sm font-medium text-gray-700">Phone (e.g. +92xxxxxxxxxx)</label>
                            <input type="text" value="{{ $employee->phone }}" pattern="^\+92\d{10}$" title="Phone number must be start with +92 followed by the 10 digits of your phone number" id="phone" name="phone"
                                class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                        </div>
                    </div>
                    <div class="mb-4">
